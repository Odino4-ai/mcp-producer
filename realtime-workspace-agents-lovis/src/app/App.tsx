...
// Multi-agent task system imports
import { multiAgentTaskScenario, multiAgentSystemName } from "@/app/agentConfigs/multiAgentTaskSystem";

// Map used by connect logic for scenarios defined via the SDK (multi-agent system)
const sdkScenarioMap: Record<string, RealtimeAgent[]> = {
  multiAgentTaskManagement: multiAgentTaskScenario,
};
...
function App() {
  ...
  useEffect(() => {
    // Multi-agent scenario bootstrap (support both old and new scenarios)
  ...
  const connectToRealtime = async () => {
    ...
    // Support both SDK scenario map and direct agent sets
    const agents = sdkScenarioMap[agentSetKey] || allAgentSets[agentSetKey];
    
    if (agents && selectedAgentConfigSet) {
      ...
      try {
        ...
        // Use the selected agent config set directly
        ...
        const guardrails = [createModerationGuardrail(multiAgentSystemName)];
  ...
  const [isAgentVisualizerVisible, setIsAgentVisualizerVisible] = useState(true);
  ...
  useEffect(() => {
    ...
    const storedAgentVisualizer = localStorage.getItem('agentVisualizerVisible');
    if (storedAgentVisualizer !== null) {
      setIsAgentVisualizerVisible(storedAgentVisualizer === 'true');
    } else {
      setIsAgentVisualizerVisible(true);
    }
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('agentVisualizerVisible', isAgentVisualizerVisible.toString());
    ...
    <div className="text-base flex flex-col h-screen bg-gray-100 text-gray-800 relative">
      <div className="p-5 text-lg font-semibold flex justify-between items-center">
        ...
        >
          ...
          <div>
            {multiAgentSystemName} <span className="text-gray-500">System</span>
          </div>
        </div>
      ...
      <div className="flex flex-1 gap-2 px-2 overflow-hidden relative">
        ...
        <AgentVisualizer isVisible={isAgentVisualizerVisible} />
      </div>
      ...
      <BottomToolbar
        ...
        isAgentVisualizerVisible={isAgentVisualizerVisible}
        setIsAgentVisualizerVisible={setIsAgentVisualizerVisible}
      />
    </div>
  );
}
